...
function fn_insert(){
	if( !chkNull($("#name").val()) ) {
		...
		if( chkNull($("#sapCode").val()) ) {
			var URL = "../material/selectMaterialDataCountAjax";
			$.ajax({
				type:"POST",
				url:URL,
				//동기 방식 사용 설정
		...
		if( count > 0 ) {
			...
			$('#lab_loading').show();
			var URL = "../material/insertMaterialAjax";
	...
	$.ajax({
		...
		success: function(data){
			...
			jsonData.list.forEach(function(item){
				...
				var row = '<tr onClick="fn_setMaterialPopupData(\''+item.SAP_CODE+'\', \''+item.NAME+'\', \''+item.UNIT+'\', \''+item.KEEP_CONDITION+'\', \''+item.WIDTH+'\', \''+item.LENGTH+'\', \''+item.HEIGHT+'\', \''+item.TOTAL_WEIGHT+'\', \''+item.STANDARD+'\', \''+item.ORIGIN+'\', \''+item.EXPIRATION_DATE+'\')">';
...
function fn_setMaterialPopupData(SAP_CODE, NAME, UNIT, KEEP_CONDITION, WIDTH, LENGTH, HEIGHT, TOTAL_WEIGHT, STANDARD, ORIGIN, EXPIRATION_DATE) {
	...
	$("#keepCondition_label").html($("#keepCondition").selectedTexts());
	$("#width").val(nvl(WIDTH,'0'));
	$("#length").val(nvl(LENGTH,'0'));
	$("#height").val(nvl(HEIGHT,'0'));
	$("#weight").val(nvl(TOTAL_WEIGHT,''));
	$("#standard").val(nvl(STANDARD,''));
	$("#origin").val(nvl(ORIGIN,''));
	$("#expireDate").val(nvl(EXPIRATION_DATE,''));
		...
		<div class="group01 mt20">
			...
			<div class="main_tbl">
				<table class="insert_proc01">
					...
					<tbody>
						<tr>
							<th style="border-left: none;">ERP 코드</th>
							<td>
							...
							<td>
								<input type="text" value=""  style="width:302px;" name="name" id="name" placeholder="원료명을 입력하세요."/>
							</td>
						</tr>
						<tr>
							<th style="border-left: none;">단가</th>
							<td>
								<input type="text"  style="width:149px;" name="price" id="price" placeholder="숫자만 입력하세요." onkeyup="checkDecNum(this)" value="0">
							</td>
						...
						<tr>
							...
							<td>
								<input type="text"  style="width:49px;" name="width" id="width" onkeyup="checkDecNum(this)"> /
								<input type="text"  style="width:49px;" name="length" id="length" onkeyup="checkDecNum(this)"> /
								<input type="text"  style="width:49px;" name="height" id="height" onkeyup="checkDecNum(this)">
							</td>
						</tr>
						<tr>
							<th style="border-left: none;">중량<span class="mandatory">*</span></th>
							<td>
								<input type="text" style="width:149px;" name="weight" id="weight" placeholder="숫자만 입력하세요." onkeyup="checkDecNum(this)">
							</td>
							<th style="border-left: none;">규격<span class="mandatory">*</span></th>
							<td>
								<input type="text"  style="width:302px;" name="standard" id="standard">
							</td>
						</tr>
						<tr>
							<th style="border-left: none;">원산지<span class="mandatory">*</span></th>
							<td>
								<input type="text"  style="width:302px;" name="origin" id="origin">
							</td>
							<th style="border-left: none;">소비기한<span class="mandatory">*</span></th>
							<td>
								<input type="text"  style="width:149px;" name="expireDate" id="expireDate">
							</td>
						</tr>
						<tr>
							<th style="border-left: none;">원료구분상세<span class="mandatory">*</span></th>
							<td colspan="3">
								<input class="" id="selectTxtFull" name="selectTxtFull" type="text" style="width: 302px; float: left" readonly>
						...
						<tr>
							...
							<td colspan="3">
								<input type="text"  style="width:302px;" name="supplier" id="supplier">
							</td>
						</tr>
					</tbody>					
				</table>	
			...
			<div class="main_tbl">
				...
				<div class="btn_box_con4">
					<button class="btn_admin_navi" onclick="javascript:fn_insert();">저장</button>
