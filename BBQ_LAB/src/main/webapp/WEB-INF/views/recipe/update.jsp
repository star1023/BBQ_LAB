			...
			if(newValid) {
				...
				if( itemName.length > 0 || itemCompCount.length > 0 || itemCompUnit.length > 0 || itemUseCount.length > 0  || itemUseUnit.length > 0 || itemPrice.length > 0 ) {
					...
					if( !chkNull(itemName) ) {
						...
						alert("사입품 구성품수량을 입력해주세요.");
						newValid = false;
					} else if( !chkNull(itemCompUnit) ) {
						alert("사입품 구성품단위를 선택해주세요.");
						newValid = false;
					} else if( !chkNull(itemUseCount) ) {
						alert("사입품 사용량을 등록해주세요.");
						newValid = false;
					} else if( !chkNull(itemUseUnit) ) {
		...
		<div class="group01 mt20">
			...
			<div class="main_tbl">
				...
				<div class="btn_box_con4">
					...
					<button class="btn_admin_red" onclick="fn_erp()">ERP 재반영</button>
					</c:if>
