...
<script type="text/javascript">
	...
	function fn_loadList(pageNo) {
		var URL = "../recipe/selectRecipeListAjax";
		...
		$.ajax({
			...
			data:{
				...
				, "searchStatus" : $("#searchStatus").selectedValues()[0]
				, "viewCount":viewCount
				, "pageNo":pageNo
			},
			dataType:"json",
			success:function(data) {
				...
				if( data.totalCount > 0 ) {
					...
					data.list.forEach(function (item) {
						if( item.IS_LAST == 'Y' ) {
							html += "<tr id=\"recipe_"+item.DOC_NO+"_"+item.VERSION_NO+"\">";	
						} else {
							html += "<tr id=\"recipe_"+item.DOC_NO+"_"+item.VERSION_NO+"\" class=\"m_version\" style=\"display: none\">";
						}
						html += "	<td>";
						...
						if( item.IS_LAST == 'Y' ) {
							...
							html += "			<button class=\"btn_doc\" onclick=\"javascript:fn_viewHistory('"+item.RECIPE_IDX+"', '"+item.DOC_NO+"')\"><img src=\"/resources/images/icon_doc05.png\">이력</button>";
							if( '${userUtil:getUserId(pageContext.request)}' == item.DOC_OWNER ) {
								if( item.STATUS == 'BOM' ) {
									html += "			<button class=\"btn_doc\" onclick=\"javascript:fn_versionUp('"+item.RECIPE_IDX+"')\"><img src=\"/resources/images/icon_doc02.png\">개정</button>";
								} else if( item.STATUS == 'BOM_ERROR' ) {
									html += "			<button class=\"btn_doc\" onclick=\"javascript:fn_erp('"+item.RECIPE_IDX+"')\"><img src=\"/resources/images/icon_doc02.png\">ERP재반영</button>";
								} else if( item.STATUS == 'COMP' ) {
									html += "			<button class=\"btn_doc\" onclick=\"javascript:fn_erp('"+item.RECIPE_IDX+"')\"><img src=\"/resources/images/icon_doc02.png\">ERP반영</button>";
								}
								if( item.STATUS == 'TMP' || item.STATUS == 'BOM_ERROR' ) {
									html += "			<button class=\"btn_doc\" onclick=\"javascript:fn_update('"+item.RECIPE_IDX+"', '"+item.DOC_NO+"')\"><img src=\"/resources/images/icon_doc03.png\">수정</button>";
								}	
							}
							html += "		</li>";
						}
	...
	function showChildVersion(imgElement){
		...
		if(elementImg == addImg){
			...
			$('tr[id*=recipe_'+docNo+']').show();
		} else {
			$(imgElement).attr('src', $(imgElement).attr('src').replace('_m_', '_add_'));
			$('tr[id*=recipe_'+docNo+']').toArray().forEach(function(v, i){
				if(i != 0){
					$(v).hide();
				}
			})
	...
	function fn_insertForm() {
		window.location.href = "../recipe/insert";
	}
	
	function fn_view(idx) {
		window.location.href = "../recipe/view?idx="+idx;
	}
	...
	function fn_viewHistory(idx, docNo) {
		var URL = "../recipe/selectHistoryAjax";
		$.ajax({
			type:"POST",
			url:URL,
			data:{
					...
					<li>
						...
						<dd >
							<div class="selectbox" style="width:150px;">  
								<label for="searchStatus" id="searchStatus_label">선택</label> 
								<select name="searchStatus" id="searchStatus">
									<option value="">선택</option>
									<option value="TMP">임시저장</option>
									<option value="COMP">등록</option>
									<option value="BOM_ERROR">ERP반영오류</option>
									<option value="BOM">ERP반영완료</option>
								</select>
							</div>
						</dd>
					</li>
