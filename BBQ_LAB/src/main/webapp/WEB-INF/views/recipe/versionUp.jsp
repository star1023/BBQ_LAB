...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#plant").selectOptions('${recipeData.PLANT}');
		...
		$.ajax({
			...
			success: function(data){
				...
				jsonData.list.forEach(function(item){
					...
					var row = '<tr onClick="setMaterialPopupData(\''+$('#targetID').val()+'\', \''+nvl(item.SAP_CODE,'')+'\', \''+item.NAME+'\', \''+item.UNIT+'\')">';
					//parentRowId, itemImNo, itemSAPCode, itemName, itemUnitPrice
					row += '<td></td>';
					//row += '<Td>'+item.companyCode+'('+item.plant+')'+'</Td>';\
	...
	function setMaterialPopupData(parentRowId, itemSAPCode, itemName, itemUnit){
		var isDup = false;
		...
		if( isDup ) {
			...
			$('#'+parentRowId + ' input[name=itemCompCount]').val("");
			$('#'+parentRowId + ' select[name=itemCompUnit]').val(itemUnit.toLowerCase());
	...
	function fn_versionUpTmp(){
		...
		formData.append("currentIdx",$("#currentIdx").val());
		...
		formData.append("unit",$("#unit").selectedValues());
			...
			if(newValid) {
				...
				if( itemName.length > 0 || itemCompCount.length > 0 || itemCompUnit.length > 0 || itemUseCount.length > 0  || itemUseUnit.length > 0 || itemPrice.length > 0 ) {
					...
					if( !chkNull(itemName) ) {
						...
						alert(itemName+"사입품 사용량을 등록해주세요.");
						newValid = false;
					} else if( !chkNull(itemUseUnit) ) {
		...
		URL = "../recipe/versionUpTmpRecipeAjax";
		$.ajax({
			type:"POST",
			url:URL,
			data: formData,
	...
	function fn_versionUp(){
		if( !chkNull($("#plant").selectedValues()[0]) ) {
			...
			URL = "../recipe/versionUpRecipeAjax";
			$.ajax({
				type:"POST",
				url:URL,
				data: formData,
...
<div class="wrap_in" id="fixNextTag">
	<span class="path">사전원가서 개정&nbsp;&nbsp; <img
		src="/resources/images/icon_path.png" style="vertical-align: middle" />&nbsp;&nbsp;사전원가서&nbsp;&nbsp; <img src="/resources/images/icon_path.png"
					...
					<table class="insert_proc01">
						...
						<tbody>
							<tr>
								...
								<td>
									<input type="hidden" name="currentIdx" id="currentIdx" value="${recipeData.RECIPE_IDX}">
									<input type="hidden" name="docNo" id="docNo" value="${recipeData.DOC_NO}">
									<input type="hidden" name="versionNo" id="versionNo" value="${recipeData.VERSION_NO}">
									<input type="hidden" name="productCode" id="productCode" value="${recipeData.PRODUCT_CODE}">
									${recipeData.PRODUCT_CODE}
								</td>
								<th style="border-left: none;">제품명<span class="mandatory">*</span></th>
								<td>
									<input type="hidden" name="productName" id="productName" value="${recipeData.PRODUCT_NAME}">
							...
							<tr>
								...
								<td>
									<input type="text" style="width: 100px; float: left" name="productCount" id="productCount" placeholder="수량 입력." onkeyup="clearNoNum(this)" value="${recipeData.PRODUCT_COUNT}"/>									
								</td>
					...
					<table id="matTable" class="tbl05">
						...
						<tbody id="mat_tbody" name="mat_tbody">
							...
							<tr id="mat_tr_${status.count}" class="temp_color">
								...
								<td>
									<input type="text" name="itemSapCode" style="width: 100px" value="${materialList.SAP_CODE}"/>
									<button class="btn_code_search2" onclick="openMaterialPopup(this,'mat')"></button>
								</td>
								<td>
									<input type="text" name="itemName" style="width: 95%" readonly="readonly" class="read_only" value="${materialList.ITEM_NAME}"/>
								</td>
								<td>
									<input type="text" name="itemCompCount" style="width: 100%" class="" onkeyup="clearNoNum(this)" value="${materialList.ITEM_COUNT}"/>
								</td>
								<td>
									<!-- <div class="selectbox req" style="width:100%;">  
										...
										<select id="itemCompUnit" name="itemCompUnit" style="width:100%;">
											...
											<option value="ea" ${materialList.ITEM_UNIT == 'ea' or materialList.ITEM_UNIT == 'EA' ? 'selected' : ''}>개</option>
											<option value="g" ${materialList.ITEM_UNIT == 'g' or materialList.ITEM_UNIT == 'G' ? 'selected' : ''}>그램</option>
										</select>
									<!--  </div> -->	
								</td>
								<td>
									<input type="text" name="itemUseCount" style="width: 100%" onkeyup="clearNoNum(this)" value="${materialList.USED_COUNT}"/>
								</td>
								<td>
									<!--<div class="selectbox req" style="width:100%;">  
										...
										<select id="itemUseUnit" name="itemUseUnit" style="width:100%;">
											<option value="">선택</option>
											<option value="ea" ${materialList.USED_UNIT == 'ea' or materialList.USED_UNIT == 'EA' ? 'selected' : ''}>개</option>
											<option value="g" ${materialList.USED_UNIT == 'g' or materialList.USED_UNIT == 'G' ? 'selected' : ''}>그램</option>
										</select>
									<!--</div>-->
								</td>
							</tr>
						...
						style="display: none">
							<tr id="mat_tmp_tr_1" class="temp_color">
								...
								<td>
									<input type="text" name="itemCompCount" style="width: 100%" class="" onkeyup="clearNoNum(this)"/>
								</td>
								<td>
									<!--<div class="selectbox req" style="width:100%;">  
										<label for="itemCompUnit" id="itemCompUnit_label"> 선택</label> -->
								...
								<td>
									<input type="text" name="itemUseCount" style="width: 100%" onkeyup="clearNoNum(this)"/>
								</td>
								<td>
									<!--<div class="selectbox req" style="width:100%;">  
										<label for="itemUseUnit" id="itemUseUnit_label"> 선택</label> -->
				...
				<table id="new_Table" class="tbl05">
					...
					<tbody id="new_tbody" name="new_tbody">
						...
						<tr id="new_tr_${status.count}" class="temp_color">
							...
							<td>
								<input type="text" name="itemName" style="width: 100%" class="code_tbl" value="${purchaseList.ITEM_NAME}"/>
							</td>
							<td>
								<input type="text" name="itemCompCount" style="width: 100%" onkeyup="clearNoNum(this)" value="${purchaseList.ITEM_COUNT}"/>
							</td>
							<td>
								<!--<div class="selectbox req" style="width:100%;">  
									<label for="itemCompUnit" id="itemCompUnit_label"> 선택</label> -->
							...
							<td>
								<input type="text" name="itemUseCount" style="width: 100%" class="" onkeyup="clearNoNum(this)" value="${purchaseList.USED_COUNT}"/>
							</td>
							<td>
								<!--<div class="selectbox req" style="width:100%;">  
									<label for="itemUseUnit" id="itemUseUnit_label"> 선택</label> -->
					...
					<tbody id="new_tbody_temp" name="new_tbody_temp"
						...
						<tr id="new_tmp_tr_1" class="temp_color">
							...
							<td>
								<input type="text" name="itemCompCount" style="width: 100%" onkeyup="clearNoNum(this)"/>
							</td>
							<td>
								<!--<div class="selectbox req" style="width:100%;">  
									<label for="itemCompUnit" id="itemCompUnit_label"> 선택</label> -->
							...
							<td>
								<input type="text" name="itemUseCount" style="width: 100%" class="" onkeyup="clearNoNum(this)"/>
							</td>
							<td>
								<!--<div class="selectbox req" style="width:100%;">  
									<label for="itemUseUnit" id="itemUseUnit_label"> 선택</label> -->
