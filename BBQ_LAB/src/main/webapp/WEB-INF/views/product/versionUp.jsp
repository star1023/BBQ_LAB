	...
	function fn_insertTmp() {
		if( !chkNull($("#title").val()) ) {
			...
			$('tr[id^=improve_pur_tr]').toArray().forEach(function(purposeRow){
				...
				var itemImprove = $('#'+ rowId + ' input[name=itemImprove]').val();
				var itemExist = $('#'+ rowId + ' input[name=itemExist]').val();
				var itemNote = $('#'+ rowId + ' input[name=itemNote]').val();
				if( !(itemImprove == '' && itemExist == '' && itemNote == '') ) {
					itemImproveArr.push(itemImprove);
					itemExistArr.push(itemExist);
			...
			$('tr[id^=improve_tr]').toArray().forEach(function(purposeRow){
				var rowId = $(purposeRow).attr('id');
				var improve = $('#'+ rowId + ' input[name=improve]').val();
				if( improve != '' ) {
					improveArr.push(improve);	
				}
			});		
			...
			$.ajax({
				...
				success:function(result) {
					if( result.RESULT > 0 ) {
						alert($("#productName").val()+"(버전 : "+$("#versionNo").val()+")"+"는 존재하는 문서입니다.");
						$('#lab_loading').hide();
						return;
					} else {
	...
	function fn_previewDataBinding(popup) {
	 ...
	 // ▼ [첨부파일 유형] 미리보기 바인딩 (value → 라벨 텍스트로)
	    (() => {
	      ...
	      const $wrap = $doc.getElementById('wrapper_prev_fileType'); // 있으면 사용

	      if (types.length) {
	    	// 쉼표+공백으로 연결, 마지막에는 자동으로 안 붙음
	    	$prev.innerHTML = types.map(esc).join(', ');
	    	if ($wrap) $wrap.style.display = 'table-row'; // 또는 'block'
	      } else {
...
//	          UL이 아직 갱신되기 전이라 누락될 수 있으니 보강
	...
	function fn_openPreview() {
		var url = "/preview/productVersionUpPopup";
...
<div class="wrap_in" id="fixNextTag">
	<span class="path">
		제품완료보고서 개정&nbsp;&nbsp;
		<img src="/resources/images/icon_path.png" style="vertical-align: middle" />&nbsp;&nbsp;제품완료보고서&nbsp;&nbsp;
		<img src="/resources/images/icon_path.png" style="vertical-align: middle" />&nbsp;&nbsp;<a href="#none">${strUtil:getSystemName()}</a>
	</span>
	<section class="type01">
		<h2 style="position:relative">
			<span class="title_s">Product Version Up Doc</span><span class="title">제품완료보고서 개정</span>
					...
					<table class="insert_proc01">
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3">
									<input type="text" style="width:50px; float: left" name="versionNo" id="versionNo" value="${productData.data.VERSION_NO+1}" onkeyup="chkNum(this)"/>
								</td>
							</tr>
							<tr>
					...
					<table id="matTable" class="tbl05">
						...
						<c:forEach items="${productMaterialData}" var="productMaterialData" varStatus="status">
							...
							<tr id="newMatRow_${status.count}" class="temp_color">
								...
								<td>
									<input type="text" name="itemSapCode" style="width: 100px" class="code_tbl read_only" value="${productMaterialData.SAP_CODE}" onkeyup="checkMaterail(event,'newMat')" readonly="readonly"/>
								</td>
								<td>
					...
					<table id="matTable" class="tbl05">
						...
						<c:forEach items="${productMaterialData}" var="productMaterialData" varStatus="status">
							...
							<tr id="matRow_${status.count}" class="temp_color">
								...
								<td>
									<input type="hidden" name="itemMatIdx" style="width: 100px" class="code_tbl" value="${productMaterialData.MATERIAL_IDX}"/>
									<input type="text" name="itemMatCode" style="width: 100px" class="code_tbl read_only" value="${productMaterialData.MATERIAL_CODE}" onkeyup="checkMaterail(event,'mat')" readonly="readonly"/>
								</td>
								<td>
...
<div class="white_content" id="approval_dialog">
	...
	<div class="modal" style="	margin-left:-500px;width:1000px;height: 550px;margin-top:-300px">
		<h5 style="position:relative">
			<span class="title">제품완료보고서 결재 상신</span>
			<div  class="top_btn_box">
