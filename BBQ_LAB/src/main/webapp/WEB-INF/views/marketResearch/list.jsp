		...
		success:function(data) {
			...
			if( data.totalCount > 0 ) {
				...
				data.list.forEach(function (item) {
					...
					html += "	<td onclick=\"fn_view('"+item.RESEARCH_IDX+"')\" style=\"cursor:pointer;\">";
					html += "		<div class=\"ellipsis_txt tgnl\">&nbsp;&nbsp;" + nvl(item.TITLE,'&nbsp;') + "</div>";
					html += "	</td>";

					html += "	<td onclick=\"fn_view('"+item.RESEARCH_IDX+"')\" style=\"cursor:pointer;\">" + nvl(item.RESEARCH_DATE,'&nbsp;') + "</td>";

					html += "	<td onclick=\"fn_view('"+item.RESEARCH_IDX+"')\" style=\"cursor:pointer;\">" + nvl(item.STATUS_TXT,'&nbsp;') + "</td>";

					html += "	<td onclick=\"fn_view('"+item.RESEARCH_IDX+"')\" style=\"cursor:pointer;\">" + nvl(item.DOC_OWNER_NAME,'&nbsp;') + "</td>";
					...
					if( item.IS_LAST == 'Y' ) {
						...
						if( '${userUtil:getUserId(pageContext.request)}' == item.DOC_OWNER && $('#listType').val() != 'search' ) {
							...
							if( item.STATUS == 'TMP' ) {
								html += "			<button class=\"btn_doc\" onclick=\"javascript:fn_delete('"+item.RESEARCH_IDX+"')\"><img src=\"/resources/images/icon_doc04.png\">삭제</button>";
							}
						}
						html += "		</li>";
					}
...
function fn_insertForm() {
	window.location.href = "../marketResearch/insert";
}

function fn_view(idx) {
	if( $('#listType').val() != 'search' ) {
		window.location.href = "../marketResearch/view?idx="+idx;	
	}
}
...
function fn_delete(idx) {
	$('#lab_loading').show();
	var URL = "../marketResearch/deleteResearchAjax";
	$.ajax({
		type:"POST",
		url:URL,
		data:{
...
function fn_viewHistory(idx) {
	var URL = "../marketResearch/selectHistoryAjax";
	$.ajax({
		type:"POST",
		url:URL,
		data:{
				...
				<ul class="tab">
					<c:choose>
						<c:when test='${userUtil:getUserType(pageContext.request) == "LEADER"}'>
							...
							<a href="javascript:changeListType('search')" id="search"><li class="change">전체 시장조사결과보고서</li></a>
						</c:when>
						<c:when test='${userUtil:getUserType(pageContext.request) == "RESEARCHER"}'>
							<a href="javascript:changeListType('my')" id="my"><li class="select">'${userUtil:getUserName(pageContext.request)}님의 시장조사결과보고서</li></a>
							<a href="javascript:changeListType('search')" id="search"><li class="change">전체 시장조사결과보고서</li></a>
