...
function fn_apprSubmit(){
	if( $("#apprLine option").length == 0 ) {
		...
		formData.append("docIdx",'${etcData.data.ETC_IDX}');
		...
		formData.append("title", "${etcData.data.TITLE}");
...
/* 	var url = "/preview/designReportViewPopup?idx="+idx;
	...
	$('#lab_loading').show();
    fetch("/preview/etcReportViewPopup?idx=" + idx)
        .then(function(res) {
            return res.text();
        })
        .then(function(html) {
			...
			formData.append("docType", "ETC");
			formData.append("userId", "${userId}");
			formData.append("title", "${etcData.data.TITLE}_기타보고서");
            ...
            .then(function(blob) {
                ...
                a.href = url;
                a.download = "${etcData.data.TITLE}_기타보고서.pdf";
                a.click();
...
function fn_openPreview(idx) {
	var url = "/preview/etcReportViewPopup?idx="+idx;
	...
	<section class="type01">
		<h2 style="position:relative">
			...
			<div class="top_btn_box">
				<ul>
					<li>
						<c:if test="${etcData.data.STATUS == 'REG' }">
		...
		<div class="group01 mt20">
			<div class="title2"  style="display: flex; justify-content:space-between; width: 100%;">
				...
				<div class="pr15">
					<button class="btn_small_search ml5" onclick="fn_openPreview('${etcData.data.ETC_IDX}')">미리보기</button>
					<c:if test="${etcData.data.STATUS eq 'COMP' && etcData.data.DOC_OWNER eq userId}">
						<button class="btn_small_search" onclick="fn_pdfDownload('${etcData.data.ETC_IDX}')">PDF 다운로드</button>
					</c:if>
				</div>
			</div>
			<div class="main_tbl">
			...
			<div>
				<table class="insert_proc01">
					<tr>
						<td>${strUtil:getHtmlBr(etcData.data.CONTENTS)}</td>
					</tr>
				</table>
			</div>
			...
			<div class="main_tbl">
				...
				<div class="btn_box_con4">
					<c:if test="${userUtil:getUserId(pageContext.request) == etcData.data.DOC_OWNER }">
						<c:if test="${(etcData.data.STATUS == 'TMP' or etcData.data.STATUS == 'COND_APPR')}">
							<button class="btn_admin_sky" onclick="fn_update('${etcData.data.ETC_IDX}')">수정</button>
						</c:if>	
					</c:if>
